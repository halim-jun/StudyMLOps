...
class ModelPipeline:
    def __init__(self, data_path: str):
        ...
        self.logistic_clf: Optional[LogisticRegression] = None
    ...
    def train_test_split(
        self, data: pd.DataFrame, path: str, test_size: float = 0.2
    ...
    def train_model(self, data: pd.DataFrame) -> LogisticRegression:
    ...
    def predict(self, data: pd.DataFrame) -> NDArray[np.bool_ | np.int_]:
        if self.logistic_clf is None:
            raise ValueError("Model is not trained. Call train_model() before predict().")
        predicted: NDArray[np.bool_ | np.int_] = self.logistic_clf.predict(
            data.drop(columns=["extrovert"])
        )
        return predicted
